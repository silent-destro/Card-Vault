              ...
              {loading ? (
                ...
                <>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                    <a
                      href={card.googleReviewUrl || `https://google.com/search?q=${encodeURIComponent(card.businessName + " " + card.city)}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      onClick={(e) => {
                        if (reviews) {
                          // Copy review to clipboard immediately
                          navigator.clipboard.writeText(reviews[activeVariant]).catch(err => {
                            console.error("Failed to copy review:", err);
                          });
                          setCopied(true);
                          
                          // Save event and review to database
                          saveReviewToDB("google");
